...
def save_violin(figs_dir: Path, df: pd.DataFrame, higher_is_better: bool, dataset: str, arch: str) -> Path:
    ...
    for c in order:
        ...
        if arr.size > 0:
            groups.append(arr); labels.append(c)
...
def save_internals_plots(figs_dir: Path, internals_by_cond, dataset: str, arch: str):
    ...
    for cond in COND_ORDER:
        ...
        if sig_mean.size:
            ...
            ax2.plot(x, sig_mean, label=cond); ax2.fill_between(x, sig_mean-sig_std, sig_mean+sig_std, alpha=0.15)
    ...
    for cond in COND_ORDER:
        ...
        if lam_mean.size:
            x = np.arange(len(lam_mean))
            ax3.plot(x, lam_mean, label=cond); ax3.fill_between(x, lam_mean-lam_std, lam_mean+lam_std, alpha=0.15)
        if vema_mean.size:
            x = np.arange(len(vema_mean))
            ax4.plot(x, vema_mean, label=cond); ax4.fill_between(x, vema_mean-vema_std, vema_mean+vema_std, alpha=0.15)
...
def main():
    ...
    # NEW: sweep grouping by tag prefix in run folder names (e.g., VW, VEMA, SLA, SLB, SLD, SLW)
